...
const portfolioCopy = {
  en: {
    ...
    experiences: [
      { period: "Apr 2026 — Present", company: "Independent AI Engineering Projects (ongoing)", role: "AI Agent & AI Application Development", detail: "Completed three public AI systems and continue to iterate on them.", highlights: ["Built a resumable agent runtime with budgets, checkpoints, specialist review, and human validation.", "Implemented hybrid RAG with BGE-M3, BM25, RRF, grounded generation, and retrieval evaluation.", "Connected typed APIs, multi-turn state, CRM automation, observability, and product-facing interfaces."], tags: ["LLM Agents", "RAG", "Evaluation", "Full-stack"] },
    ...
    recognitions: [
      ...
      { title: "Research & Innovation", detail: "1 EI-indexed first-author paper · 1 published invention patent application (first among student inventors)" },
      { title: "Languages", detail: "CET-6 English · JLPT N2 Japanese" },
    ],
    stack: [
    ...
    notes: [
      ...
      { title: "AI Sales Lead Decision & CRM Automation System", body: "Lead interpretation and follow-up: hybrid retrieval, persistent conversations, grounded drafts, and human-reviewed CRM delivery.", topics: ["Hybrid retrieval", "Conversation state", "CRM"], slug: "ai-sales-lead-crm-automation" },
  ...
  zh: {
    ...
    experiences: [
      { period: "2026.04 — 至今", company: "个人 AI 工程项目（持续迭代）", role: "AI Agent 与 AI 应用开发", detail: "完成了三个公开 AI 系统，并持续进行工程迭代。", highlights: ["构建支持预算控制、检查点、专业角色审查与人工确认的可恢复 Agent Runtime。", "实现融合 BGE-M3、BM25、RRF、Grounded Generation 与检索评测的混合 RAG 链路。", "打通强类型 API、多轮状态、CRM 自动化、可观测性与面向用户的产品界面。"], tags: ["LLM Agent", "RAG", "评测", "全栈开发"] },
      ...
      { period: "2023.06 — 2025.06", company: "电子科技大学", role: "科研 · 物理先验引导的自由空间光通信链路建模与性能预测", detail: "面向自由空间光通信链路，结合物理先验与数据驱动模型开展性能预测。", highlights: ["基于 MATLAB 仿真搭建训练数据流水线，以残差网络学习理论模型与仿真的偏差。", "平均功率预测误差由 15.1% 降至 8.7%，R² 达 0.95；条件归一化流使 Wasserstein 距离下降 15%，单工况推理耗时由 12.5 s 降至 3 s（约 4.2 倍）。"], tags: ["物理先验", "MATLAB", "模型评测"] },
    ],
    recognitions: [
      { title: "优秀研究生", detail: "研究生学业二等奖学金" },
      { title: "省一等奖", detail: "全国大学生数学建模竞赛" },
      { title: "MCM H 奖", detail: "美国大学生数学建模竞赛 · 队长" },
      { title: "科研与创新", detail: "EI 期刊第一作者论文 1 篇 · 发明专利公开 1 项（学生第一）" },
      { title: "语言能力", detail: "英语六级 · 日语 N2" },
    ],
    stack: [
    ...
    notes: [
      ...
      { title: "AI 销售线索决策与 CRM 自动化系统", body: "从线索理解到客户跟进：说明混合检索、多轮状态、有据草稿与人工审核后的 CRM 同步如何组成完整流程。", topics: ["混合检索", "对话状态", "CRM"], slug: "ai-sales-lead-crm-automation" },
    ...
    <div className={`portfolio-root ${theme} lang-${language}`} onPointerMove={updateGlow}>
      ...
      <main className="portfolio-layout" id="home">
        <aside className="profile-column"><div className="profile-card reveal-card">
          ...
          <div className="profile-socials"><a href={profile.github} target="_blank" rel="noreferrer" aria-label="GitHub"><Github size={20} /></a><a href={`mailto:${profile.email}`} aria-label="Email"><Mail size={20} /></a><a href="/resume-public.pdf?v=20260916-3" target="_blank" aria-label={ui.resume}><FileText size={20} /></a></div>
            ...
            <div className="education-recognition-grid">
              ...
              <article className="education-card recognition-panel tilt-card">
                ...
                <div className="recognition-list">{ui.recognitions.map((item, index) => {
                  const Icon = index === 3 ? Lightbulb : index === 4 ? Languages : Medal;
